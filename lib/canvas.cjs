...
function mostrarResultados(tiempoVuelo, distanciaTotal, alturaMaxima, velocidadInicial, aceleracion) {
    ...
    resultadosDiv.innerHTML = `
        <h3>Resultados de la Simulación de la Catapulta</h3>
        <p>Tiempo de Vuelo: \\( t = ${tiempoVuelo.toFixed(2)} \, \\text{s} \\)</p>
        <p>Distancia Total: \\( d = ${distanciaTotal.toFixed(2)} \, \\text{m} \\)</p>
        <p>Altura Máxima: \\( h_{max} = ${alturaMaxima.toFixed(2)} \, \\text{m} \\)</p>
        <p>Velocidad Inicial: \\( v_{0} = ${velocidadInicial.toFixed(2)} \, \\text{m/s} \\)</p>
        <p>Aceleración: \\( a = ${aceleracion} \, \\text{m/s}^{2} \\)</p>
        
        <h4>Fórmulas Usadas</h4>
        <p>Tiempo de Vuelo: \\( t = \\sqrt{\\frac{2h_{max}}{g}} \\)</p>
        <p>Distancia Total: \\( d = \\sqrt{x_{max}^2 + h_{max}^2} \\)</p>
        <p>Altura Máxima: \\( h_{max} = \\frac{peso}{100} \\cdot (\\theta - 110) \\cdot \\frac{longitudBrazo}{200} \\)</p>
        <p>Velocidad Inicial: \\( v_{0} = \\sqrt{2gh_{max}} \\)</p>
        <p>Aceleración: \\( a = g = 9.81 \, \\text{m/s}^2 \\)</p>
    `;
